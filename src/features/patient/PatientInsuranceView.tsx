...
import { ShieldCheck, Plus, Upload, Eye, Lock, ChevronDown } from "lucide-react";
          ...
          {policies.map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                {expandedId === p.id && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden border-t border-border">
                    <div className="space-y-1.5 p-3.5 text-sm">
                      ...
                      <div className="flex justify-between"><span className="text-muted-foreground">Documents</span><span className="font-medium">2 uploaded</span></div>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              <div className="flex gap-2 border-t border-border p-3">
                ...
                <button onClick={() => { setExpandedId(p.id); }} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-muted py-2 text-sm font-medium"><Upload className="size-4" /> Policy Docs</button>
              </div>
            </motion.div>
          ))}
        </div>
