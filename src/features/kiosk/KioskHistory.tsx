...
export function KioskHistory() {
  ...
  return (
    <div className="app-shell min-h-dvh bg-card">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold">Clinical History</p>
            <p className="text-xs text-muted-foreground">
              {historyMode === "ayush" ? "AYUSH · Dashavidha Pariksha" : "Step 3 of 5"}
            </p>
          </div>
          <button onClick={handleAyushMode}
            ...
            {historyMode === "ayush" ? "AYUSH ON" : "AYUSH Off"}
          </button>
        </div>
      ...
      {/* Red flag banner */}
      {redFlags.length > 0 && (
        <div className="mx-4 mt-3 flex items-center gap-2 rounded-2xl border border-red-500/30 bg-red-500/10 p-3">
          <ShieldAlert className="size-5 shrink-0 text-red-600" />
          <div className="text-xs">
            <p className="font-bold text-red-700">Priority triage recommended</p>
            <p className="text-red-600/80">{redFlags[0]} — flagged for urgent review</p>
          </div>
        </div>
      )}
      ...
      <div className="mx-4 mt-3 flex items-center gap-1.5 rounded-full bg-muted/70 p-1">
        {([
          ...
          { key: "text", icon: MessageSquareText, label: "Text" },
                ...
                <div className="mt-2.5 flex gap-2">
                  {inputMode !== "touch" && (
                    <Button variant="outline" className="h-10 flex-1" onClick={startVoiceInput} disabled={listening}>
                      {listening ? <><RefreshCw className="size-4 animate-spin" /> Listening...</> : <><Mic className="size-4" /> Speak an answer</>}
                    </Button>
                  )}
