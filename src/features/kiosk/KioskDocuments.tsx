...
export function KioskDocuments() {
  ...
  const handleAdd = () => {
    ...
    addKioskDocument(currentDoc);
    const next = [...docs, currentDoc];
    setDocs(next);
    setCurrentDoc(null);
    setPhase("idle");
  };
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold">Document Digitization</p>
        ...
        {phase === "review" && currentDoc && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mt-4 flex gap-2">
              <Button variant="outline" className="h-12 flex-1" onClick={() => { setCurrentDoc(null); setPhase("idle"); }}>
                Rescan
              </Button>
