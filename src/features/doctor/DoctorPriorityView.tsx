...
export function DoctorPriorityView() {
  ...
  const sorted = [...demoPatients].sort((a, b) =>
  ...
  const urgentCount = demoPatients.filter((p) => p.redFlagLevel === "URGENT").length;
    ...
    <DoctorPageShell title="Priority Queue" currentTab="DOCTOR_PRIORITY" sub="Sorted by clinical urgency">
      ...
      {sorted.length === 0 && (
        <div className="py-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">No patients in queue.</p>
        </div>
      )}
    </DoctorPageShell>
  );
