...
const tabs = [
  { key: "DOCTOR_HOME", label: "Dashboard", icon: LayoutDashboard },
  { key: "DOCTOR_QUEUE", label: "Patients", icon: Users },
  { key: "DOCTOR_PRIORITY", label: "Queue", icon: Activity },
  { key: "DOCTOR_NOTES", label: "Notes", icon: FileText },
  { key: "DOCTOR_SETTINGS", label: "Profile", icon: User },
];

export function DoctorBottomNav({ current }: { current: string }) {
  const setView = useAppStore((s) => s.setView);
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto w-full max-w-[640px] border-t border-border bg-card/95 backdrop-blur-md pb-safe" aria-label="Doctor navigation">
