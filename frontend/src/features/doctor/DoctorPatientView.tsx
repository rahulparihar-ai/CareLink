...
export function DoctorPatientView() {
  ...
  const evidence = useAppStore((s) =>
    s.kioskSession?.summary?.sourceEvidence?.length
      ? s.kioskSession.summary.sourceEvidence
      : []
  );
  ...
  const regenerateAi = async () => {
    const kiosk = useAppStore.getState().kioskSession;
    if (!kiosk?.clinicalHistory && !kiosk?.conversationTurns?.length) {
      return;
    }
    setRegenerating(true);
    const summary = await SUMMARY_SERVICE.generate(
      patient.id,
      kiosk.clinicalHistory,
      kiosk.documents,
      kiosk.redFlags,
      kiosk.historyMode,
