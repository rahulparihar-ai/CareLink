    ...
    <DoctorPageShell title="Clinical History" currentTab="DOCTOR_HOME">
      {!summary ? (
        <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
          ...
          <p className="text-xs text-muted-foreground">Patient history will appear here once captured via the Care Link intake.</p>
        ...
        <div className="space-y-3 pb-24">
          ...
          {(intake?.conversationTurns?.length ?? 0) > 0 && (
            <ClinicalCard icon={MessageSquareText} iconTone="text-sky-600 bg-sky-500/10" title="Interview Transcript">
              ...
              >
                {showTranscript ? "Hide transcript" : `View original conversation (${intake?.conversationTurns?.length ?? 0} turns)`}
                <ChevronDown className={cn("size-4 transition-transform", showTranscript && "rotate-180")} />
              </button>
              {showTranscript && (
                <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
                  {intake?.conversationTurns.map((turn) => (
