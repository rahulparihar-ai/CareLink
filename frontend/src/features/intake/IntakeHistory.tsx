...
export function IntakeHistory() {
  ...
  const setIntakeInputMode = useAppStore((s) => s.setIntakeInputMode);
  const setIntakeHistoryMode = useAppStore((s) => s.setIntakeHistoryMode);
  ...
  const historyMode = intake?.historyMode ?? "allopathic";
  const inputMode = intake?.inputMode ?? "voice";
    ...
    setTimeout(() => {
      ...
      if (nextIdx < allQuestions.length) {
        ...
        setTimeout(() => {
          ...
          setIntakePhase("documents");
          setTimeout(() => setView("INTAKE_DOCUMENTS"), 400);
        }, 1200);
      }
    }, 500);
  };
    ...
    <div className="app-shell min-h-dvh bg-card">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 py-3">
          <button onClick={() => { setIntakePhase("consent"); setView("INTAKE_CONSENT"); }} aria-label="Back" className="flex size-9 items-center justify-center rounded-full bg-muted text-foreground">
            <ArrowLeft className="size-5" />
          </button>
          <div className="flex-1">
            <p className="text-sm font-bold">{t("clinical.historyHeader")}</p>
            <p className="text-xs text-muted-foreground">
              {historyMode === "ayush" ? t("clinical.ayushMode") : t("clinical.historyStep")}
            </p>
          </div>
          <button onClick={handleAyushMode}
            ...
            {historyMode === "ayush" ? t("clinical.ayushOn") : t("clinical.ayushOff")}
          </button>
        </div>
        <IntakeProgress phase="history" />
      </header>
      ...
      {redFlags.length > 0 && (
        <div className="mx-4 mt-3 flex items-start gap-2 rounded-2xl border border-red-500/30 bg-red-500/10 p-3">
          ...
          <div className="text-xs">
            <p className="font-bold text-red-700">{t("clinical.flagTriage")}</p>
      ...
      <div className="mx-4 mt-3 flex items-center gap-1.5 rounded-full bg-muted/70 p-1">
        {([
          { key: "voice", icon: Mic, label: t("clinical.modeVoice") },
          { key: "touch", icon: Hand, label: t("clinical.modeTouch") },
          { key: "text", icon: MessageSquareText, label: t("clinical.modeText") },
        ] as const).map(({ key, icon: Icon, label }) => (
          <button key={key} onClick={() => { setIntakeInputMode(key); setShowTouchInput(false); }}
