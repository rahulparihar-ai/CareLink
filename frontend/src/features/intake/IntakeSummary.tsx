...
export function IntakeSummary() {
    ...
    const gen = async () => {
      try {
        const res = await SUMMARY_SERVICE.generate(
          intake?.patientId ?? generatePatientId(),
          intake?.clinicalHistory ?? null,
          intake?.documents ?? [],
          intake?.redFlags ?? [],
          intake?.historyMode ?? "allopathic",
  ...
  const handleBack = () => {
    ...
    setView("INTAKE_DOCUMENTS");
  };

  const handleConfirm = () => {
    setIntakePhase("complete");
    setView("INTAKE_COMPLETE");
  };
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold">{t("clinical.summaryHeader")}</p>
            <p className="text-xs text-muted-foreground">{t("clinical.summaryStep")}</p>
          </div>
          <div className="flex size-9 items-center justify-center rounded-full bg-primary/10 text-primary">
            <ScanLine className="size-5" />
          </div>
        </div>
        <IntakeProgress phase="summary" />
      </header>
