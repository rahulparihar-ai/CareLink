...
export function IntakeIdentify() {
  const setView = useAppStore((s) => s.setView);
  const setRole = useAppStore((s) => s.setRole);
  const setPatientProfile = useAppStore((s) => s.setPatientProfile);
  const setIntakePatientId = useAppStore((s) => s.setIntakePatientId);
  ...
  const handleContinue = async () => {
    ...
    setError("");
if (method === "abha") {
      if (!abhaId.trim()) { setError(t("clinical.enterAbha")); setLoading(false); return; }
      ...
      if (!newName.trim()) { setError(t("clinical.enterName")); setLoading(false); return; }
      const p = useAppStore.getState().patientProfile;
      setPatientProfile({ ...p!, name: newName.trim() });
    }
setRole("PATIENT");
    setIntakePatientId(generatePatientId());
    setView("INTAKE_CONSENT");
  };
    ...
    <div className="app-shell min-h-dvh bg-card">
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md px-4 py-3">
        <div className="flex items-center gap-3">
          <button onClick={() => { setView("INTAKE_HOME"); }} aria-label="Back" className="flex size-9 items-center justify-center rounded-full bg-muted text-foreground">
            <ArrowLeft className="size-5" />
          </button>
<div>
            <p className="text-sm font-bold">{t("clinical.identifyHeader")}</p>
...
<p className="mb-4 text-sm font-semibold">{t("clinical.chooseId")}</p>
        <div className="space-y-3">
          <MethodCard active={method === "abha"} onClick={() => setMethod("abha")}
            ...
            title={t("clinical.methodAbha")} sub={t("clinical.methodAbhaSub")}
          ...
          <MethodCard active={method === "aadhaar"} onClick={() => setMethod("aadhaar")}
            ...
            title={t("clinical.methodQr")} sub={t("clinical.methodQrSub")}
          ...
          <MethodCard active={method === "new"} onClick={() => setMethod("new")}
            ...
            title={t("clinical.methodNew")} sub={t("clinical.methodNewSub")}
        ...
        <motion.div layout className="mt-4 rounded-2xl border border-border bg-background p-4 card-soft">
          {method === "abha" && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground">{t("clinical.abhaField")}</label>
              <Input value={abhaId} onChange={(e) => setAbhaId(e.target.value)}
                placeholder={t("clinical.abhaPlaceholder")} className="mt-1.5 h-12" />
              <p className="mt-1.5 text-[11px] text-muted-foreground">{t("clinical.abhaExample")}</p>
            </div>
          )}
          {method === "aadhaar" && (
            <div className="text-center">
              {aadhaarFilled ? (
                <div className="flex flex-col items-center py-2">
                  ...
                  <p className="mt-2 text-sm font-semibold">{t("clinical.qrVerified")}</p>
                  <p className="text-xs text-muted-foreground">{t("clinical.identityLinked")}</p>
                </div>
              ) : (
                <button onClick={handleContinue} disabled={loading}
                  ...
                  <p className="mt-2 text-sm font-semibold">{t("clinical.scanQr")}</p>
                  <p className="text-xs text-muted-foreground">{t("clinical.positionQr")}</p>
                </button>
              )}
            </div>
          )}
          {method === "new" && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground">{t("clinical.patientName")}</label>
              <Input value={newName} onChange={(e) => setNewName(e.target.value)}
                placeholder={t("clinical.patientNamePlaceholder")} className="mt-1.5 h-12" />
              <p className="mt-1.5 text-[11px] text-muted-foreground">
                {t("clinical.nameLater")}
              </p>
            </div>
          )}
        </motion.div>
      ...
      <div className="sticky bottom-0 border-t border-border bg-card/95 px-4 py-3 backdrop-blur-md">
        <Button className="h-12 w-full rounded-xl text-sm font-semibold" onClick={handleContinue} disabled={loading}>
          {method === "aadhaar" && !aadhaarFilled ? t("clinical.scanQrCta") : t("clinical.continue")}
