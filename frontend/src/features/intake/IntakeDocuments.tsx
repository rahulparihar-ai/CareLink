...
export function IntakeDocuments() {
  ...
  const addIntakeDocument = useAppStore((s) => s.addIntakeDocument);

  const [phase, setPhase] = useState<Phase>("idle");
  const [currentDoc, setCurrentDoc] = useState<DocumentIntelligence | null>(null);
  const [docs, setDocs] = useState<DocumentIntelligence[]>(intake?.documents ?? []);
  ...
  const handleContinue = () => {
    setIntakePhase("summary");
    setView("INTAKE_SUMMARY");
  };
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold">{t("clinical.docsHeader")}</p>
            <p className="text-xs text-muted-foreground">{t("clinical.docsStep")}</p>
          </div>
          <div className="flex size-9 items-center justify-center rounded-full bg-primary/10 text-primary">
            <FileStack className="size-5" />
          </div>
        </div>
        <IntakeProgress phase="documents" />
      </header>
