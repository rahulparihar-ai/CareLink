...
export function KioskHome() {
  ...
  const beginFlow = () => {
    startKioskSession({ phase: "identify" });
...
<button onClick={() => setView("HOSPITAL_HOME")} className="mb-6 flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
          <ArrowLeft className="size-4" /> {t("kiosk.backToHospital")}
        </button>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="mt-4 flex flex-wrap gap-2">
          <span className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
            <Mic className="size-3" /> {t("kiosk.voice")}
          </span>
          <span className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
            <Hand className="size-3" /> {t("kiosk.touch")}
          </span>
          <span className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
            <MessageSquareText className="size-3" /> {t("kiosk.languages")}
          </span>
          <span className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
            <ShieldCheck className="size-3" /> {t("kiosk.abdm")}
          </span>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mt-6 rounded-2xl border border-border bg-background p-4 card-soft">
          <p className="mb-3 text-sm font-semibold">{t("kiosk.howTitle")}</p>
          <div className="space-y-2.5 text-[13px] text-muted-foreground">
            <StepRow n="1" text={t("kiosk.step1")} />
            <StepRow n="2" text={t("kiosk.step2")} />
            <StepRow n="3" text={t("kiosk.step3")} />
            <StepRow n="4" text={t("kiosk.step4")} />
          </div>
        </motion.div>
        ...
        <div className="mt-6">
          <Button className="h-14 w-full rounded-xl text-base font-semibold" onClick={beginFlow}>
            <ScanFace className="size-5" /> {t("kiosk.start")}
          </Button>
          <div className="mt-3 flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground">
            <ShieldCheck className="size-3.5" /> {t("kiosk.abhaNote")}
          </div>
        </div>

        {/* Language select */}
        <div className="mt-6">
          <p className="mb-2 text-sm font-semibold">{t("kiosk.selectLang")}</p>
        ...
        <div className="mt-6 flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground">
          <Building2 className="size-3.5" /> {t("kiosk.footer")}
        </div>
      </div>
    </div>
  );
