...
<div>
            <p className="text-sm font-bold">{t("kiosk.consentHeader")}</p>
...
<div className="flex items-center gap-2 text-emerald-600">
            <ShieldCheck className="size-5" />
            <p className="text-sm font-semibold">{t("kiosk.consentProtected")}</p>
          </div>
          <p className="mt-1.5 text-xs text-muted-foreground">
            {t("kiosk.consentBodyLong")}
          </p>
        </motion.div>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.05 }}
          className="mb-3 text-sm font-semibold">
          {t("kiosk.consentReview")}
        </motion.p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {opt.required && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-semibold uppercase text-primary">{t("kiosk.consentRequired")}</span>}
                    </div>
                  ...
                  <button
                    ...
                    aria-label={t("kiosk.consentListen")}
                  >
...
<div className="mt-4 flex items-center justify-center gap-1.5 text-[10px] text-muted-foreground">
          <X className="size-3" /> {t("kiosk.consentRevoke")}
        </div>
      </div>
      ...
      <div className="sticky bottom-0 border-t border-border bg-card/95 px-4 py-3 backdrop-blur-md">
        <Button className="h-12 w-full rounded-xl text-sm font-semibold" onClick={handleContinue}>
          {t("kiosk.accept")} <ChevronRight className="size-4" />
        </Button>
      </div>
    </div>
  );
