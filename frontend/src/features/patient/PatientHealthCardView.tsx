...
import { Droplets, AlertTriangle, HeartPulse, Phone, User } from "lucide-react";
      ...
      <PatientPageShell title="Health Card" currentTab="PATIENT_PROFILE" onBack={() => setView("PATIENT_PROFILE")}>
        <div className="rounded-2xl border border-border bg-card p-6 text-center card-soft">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Complete registration to generate your CareLink health card.
          </p>
        </div>
      </PatientPageShell>
    );
      ...
      <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-primary to-primary/85 text-primary-foreground shadow-xl shadow-primary/30">
        ...
        <div className="flex items-center justify-between gap-4 p-5">
          ...
          <div className="shrink-0 rounded-2xl bg-white p-2">
            <QRCodeSVG value={p?.id ?? ""} size={96} fgColor="#1a6eb5" />
          </div>
        </div>
      </div>
    </PatientPageShell>
